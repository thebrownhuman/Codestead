...
Rclone metadata and pointer operations have a fixed 120-second control deadline. For archive size `B`, each upload, readback, or download bulk leg has deadline `ceil(B / RCLONE_MIN_BULK_BYTES_PER_SECOND) + RCLONE_BULK_OVERHEAD_SECONDS`. With the defaults this is `ceil(B / 4194304) + 600` seconds. Publication fails before upload unless `2 * bulk_deadline + 600 < 14400`; retrieval likewise fails before download unless its bulk leg plus the restore/control reserve fits. The 15-second connect timeout and 60-second idle/progress timeout remain active inside every deadline.

The offsite success age may be at most 30 hours: daily cadence plus 20-minute jitter, the four-hour service runtime, and operational margin. The health timer polls hourly, so a missed daily publication alerts on the first poll after that threshold. This monitoring age does not relax the 24-hour RPO, which is measured separately by the supervised restore evidence.
