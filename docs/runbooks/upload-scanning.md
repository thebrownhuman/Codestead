...
All accepted source, text, and PDF uploads begin with `scan_status='pending'` and are not downloadable. A dedicated worker atomically leases rows with `FOR UPDATE SKIP LOCKED`, opens only a server-generated `<owner-id>/<object-uuid>` path from its read-only storage mount, verifies size and SHA-256 while streaming, and sends the byte stream to clamd using its `INSTREAM` protocol. Clean objects become `safe`; detected objects become `quarantined`. No scanner result can overwrite a deleted object or a newer worker lease.

## Service boundary

- `scan-worker` can read PostgreSQL and the object bind mount. The mount is read-only. It has no general outbound network.
...
```bash
sudo docker compose --env-file /etc/learncoding/compose.env -f /opt/learncoding/compose.yaml ps clamav scan-worker
...
Changing retry settings requires an operator review. Keep the lease longer than `CLAMD_TIMEOUT_SECONDS` plus worst-case database latency. Never set the worker mount writable to repair a queue.
...
If clamd is unhealthy, uploads remain quarantined and learning features continue. Check container health, signature-volume capacity, DNS and outbound access for signature updates, then inspect aggregate logs. Do not update rows to `safe` manually. Once clamd is healthy, restart only `scan-worker` if it has not recovered automatically; pending jobs retain their attempt count and retry schedule.
