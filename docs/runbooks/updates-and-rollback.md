...
The database sequence is mandatory: role bootstrap, restricted-role negative probes, migration, idempotent role/ACL reconciliation, seed or initial administrator work, then application-object boundary verification. A migration such as 0061 that contains `GRANT` or `REVOKE` for a named role must never be invoked alone on a pristine database; the first bootstrap creates that role, and the second reconciles every migrated object plus owner default ACLs. Failure at any stage aborts while the tunnel remains quarantined.
...
The one-time active-release schema-v2 transition is fail-closed. If a retained predecessor would otherwise be advertised as rollback-compatible, the trusted current helper must derive its exact application commit and tree from the retained release record, resolve both trees only through local Git object plumbing, compare the same versioned semantic host-operations surface, and fsync `previous-runtime-host-operations-compatibility.env` before any image pull or host-storage mutation. Missing objects, symlinks, submodules, command/environment-name/mount/network/policy drift, or malformed evidence refuse the release before non-quarantine mutation. Recovery evidence no longer accepts the legacy v1 active manifest; the first successful transition or explicit rollback publishes v2 with separate application and host-operations provenance. Do not bypass this gate to rescue an old record: retain the exact predecessor objects for attestation or restore a verified recovery point.
The retained host-operations compatibility evidence is the one-time transition attestation; a v1 active manifest alone never authorizes rollback.


`0062_mail_outbox_retention_redaction` is a forward-only authority boundary: it moves retention and redaction from the legacy direct application claimant to the ops-owner `SECURITY DEFINER` routines. Never pass `--schema-backward-compatible` across this boundary. Inspect the failed release's `mail-outbox-contract.env`; a runnable rollback command is permitted only when it records `PREVIOUS_RUNTIME_COMPATIBLE=true`, `FORWARD_ONLY_MIGRATION=none`, and the current and previous `OUTBOX_RETENTION_AUTHORITY` as `ops-owner-security-definer-v1`. A record that crosses 0062 intentionally provides no runnable compatibility command in `rollback.txt`.

`OUTBOX_WORKER_MODE=fenced-postgres-v1 cannot roll back to legacy-direct-v1`; restoring the legacy direct claimant is forward-only-refused even when unrelated application schema is compatible. A later ops-owner-aware release remains eligible to roll back to an earlier ops-owner-aware release only when both releases keep the fenced claimant and the versioned contract records compatibility.

`0064_mail_outbox_dispatch_binding` is a second forward-only authority boundary. A source at or after its approved Git introduction may run only when that exact tree contains `infra/ops/mail-outbox-dispatch-binding-capability.env` with the reviewed schema, `fenced-postgres-v1` worker mode, exact pre-provider-call payload binding, and the owner-execute/worker-column-update-only trigger privilege contract. Rollback requires matching V3 fields and matching capability blobs in the exact recorded source and previous Git trees; an operator compatibility assertion cannot substitute for them. A missing, legacy, unknown, or mismatched capability disables automatic restore before the rollback override or application Compose start. This contract does not restore blanket privileges.

`0069_mail_outbox_guarded_delivery_authority` is a third forward-only authority boundary, introduced only by reviewed commit `7eeafd73c5d41ea49526d908165e0a7cefa92097`. A source at or after that commit may release only when its exact candidate tree contains `infra/ops/mail-outbox-guarded-delivery-capability.env` as the pinned regular Git blob and declares the reviewed guarded TX1/TX2 exact-byte runtime, append-only delivery-release authority, and owner/app/worker/release-receipt least-privilege contract. V4 `mail-outbox-contract.env` evidence binds all three current values and all three previous values to the exact source and previous Git trees. Rollback is eligible only when both commits descend from 0069, both trees contain that exact capability, both worker modes remain `fenced-postgres-v1`, `PREVIOUS_RUNTIME_COMPATIBLE=true`, and `FORWARD_ONLY_MIGRATION=none`. Crossing 0069 is never a SQL rollback: a missing, tampered, wrong-mode, unknown, or mismatched capability fails before application Compose or rollback-override execution.

No authority boundary after 0069 has an approved introduction SHA. Treat every future capability-manifest schema or authority change as incompatible until its exact migration commit and additive contract version are reviewed; do not reuse an existing identifier or infer compatibility from migration presence alone.
