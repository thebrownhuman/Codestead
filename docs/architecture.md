...
PostgreSQL remains authoritative, while an HMAC user-and-session-namespaced IndexedDB outbox is the browser-durable boundary for unsynchronized lesson drafts and exam-answer mutations; `sessionStorage` is only an optional warm-page cache. The interface may say `Saved locally; Codestead will retry.` only after the IndexedDB transaction commits, and `Saved to Codestead` only after the server acknowledges the same mutation ID. There is still no service worker, OPFS authority, cold-offline launch, or install surface. Durable request receipts, compare-and-delete acknowledgement, and optimistic row versions make replay exactly-once without allowing an older acknowledgement to erase newer text. Code-draft identity includes implementation language, so DSA language switches cannot overwrite another facet. A 401/403, confirmed logout, session revocation, applicable closed-book transition, or terminal exam state purges the scoped opaque browser records; a failed sign-out preserves dirty local-only text. Server draft saves require an active/completed enrollment in a beta/verified course skill and serialize a 512-record/32 MiB per-user aggregate quota that is mirrored by a PostgreSQL trigger.
