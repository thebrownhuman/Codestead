...
The compatibility flag never authorizes rollback across `0062_mail_outbox_retention_redaction` from the ops-owner `SECURITY DEFINER` retention authority to the legacy direct authority, nor from `fenced-postgres-v1` to `legacy-direct-v1`. The versioned `mail-outbox-contract.env` records both authority and claimant transitions; `rollback.txt` omits a runnable compatibility command whenever either transition is forward-only. Only a later ops-owner-aware, fenced release whose contract records `PREVIOUS_RUNTIME_COMPATIBLE=true` may restore its exact earlier ops-owner-aware, fenced runtime.
...
After `0064_mail_outbox_dispatch_binding`, deployment also requires the exact checked-in dispatch-binding capability contract and V3 release evidence bound to the source and previous Git trees. Until the worker persists the exact adapter payload SHA-256 before every provider call and the reviewed trigger/column privilege contract is checked in, release intentionally fails before application Compose execution. Missing or unknown future contract versions are incompatible; approve and pin a later authority boundary before allowing them, and never restart an older worker merely because `--schema-backward-compatible` was supplied.
