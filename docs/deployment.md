...
Read [the secret inventory](../infra/secrets/README.md) first. This ceremony is for initial creation only: it acquires a per-directory lock, refuses to overwrite any existing database secret, and rolls back every final created by a failed run. Run it from the installed release. It stages and validates the complete inventory before publication, generates independent URL-safe hexadecimal passwords for the fixed `learncoding`, `learncoding_app`, `learncoding_migrator`, `learncoding_worker`, `learncoding_ops`, and `learncoding_backup_reporter` roles, creates `postgres_password`, `database_bootstrap_url`, `database_url`, `database_migrator_url`, `database_worker_url`, `database_ops_url`, and `database_backup_reporter_url` without trailing whitespace or control bytes, and applies the required `root:codestead-secrets` ownership and exact modes without printing a value.
...
The database security stages have a fixed order: `database-role-bootstrap` creates or resets the five fixed roles and canonical pre-migration ACLs; `database-negative-probes` verifies the restricted credentials; `migrate` runs only after those roles exist; `database-role-reconciliation` reruns the same idempotent bootstrap to reconcile table, column, routine, sequence, type, and default privileges created or changed by migration; and `database-boundary-verifier` checks application objects before runtime or public exposure. Migrations such as `0061_mail_worker_outbox_privileges.sql` directly name `learncoding_worker`, so invoking migration alone on a pristine cluster is unsupported and can fail before grant execution. The second bootstrap pass is mandatory, not an optional repair.

The compatibility flag never authorizes rollback across `0062_mail_outbox_retention_redaction` from the ops-owner `SECURITY DEFINER` retention authority to the legacy direct authority, from `fenced-postgres-v1` to `legacy-direct-v1`, or across `0069_mail_outbox_guarded_delivery_authority` to an image without its exact candidate-bound guarded-delivery capability. The versioned `mail-outbox-contract.env` records the current and previous authority, claimant, dispatch-binding, guarded-runtime, delivery-release, and privilege contracts; `rollback.txt` omits a runnable compatibility command whenever any transition is forward-only. Only an exact later compatible release whose contract records `PREVIOUS_RUNTIME_COMPATIBLE=true` and `FORWARD_ONLY_MIGRATION=none` may restore its exact earlier capability-bearing runtime.
...
That gate starts from a cluster where the restricted roles do not yet exist, runs the real role bootstrap before the first migration, and replays the bootstrap while proving it shares the database-administration lock with migration. This clean-cluster proof covers role-referencing migrations such as 0061. Its migration replay is not a substitute for the explicit release transaction's guaranteed post-migration reconciliation and application-object boundary verification.
...
`MAIL_DISPATCH_RUNTIME_DEFAULTS` records a 120-second application stop policy, a 100-second drain with an exclusive 105-second upper bound, and a 5-second pool close. The `mail-worker` container receives an exact 135-second Compose grace, leaving a 15-second platform margin beyond the application stop. Runtime preflight rejects equality, any shorter value, and every other non-135-second rendered override. This validates only the deployment envelope; worker, store, and process adoption remain separate reviewed changes.

After `0064_mail_outbox_dispatch_binding`, deployment also requires the exact checked-in dispatch-binding capability contract and V3-or-newer release evidence bound to the source and previous Git trees. Until the worker persists the exact adapter payload SHA-256 before every provider call and the reviewed trigger/column privilege contract is checked in, release intentionally fails before application Compose execution.

At and after reviewed commit `7eeafd73c5d41ea49526d908165e0a7cefa92097`, `0069_mail_outbox_guarded_delivery_authority` additionally requires the exact checked-in `infra/ops/mail-outbox-guarded-delivery-capability.env` blob. V4 evidence binds the guarded TX1/TX2 exact-byte runtime, append-only delivery-release authority, and guarded-delivery privilege contract for both the candidate and previous Git trees. Release and rollback reject a missing, tampered, wrong-mode, unknown, or mismatched capability before application Compose or rollback-override execution. `--schema-backward-compatible` never permits crossing 0069 or restarting an image without that exact capability, and neither tool reverses the migration SQL.
