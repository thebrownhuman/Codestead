...
-- after the exact reviewed 0065/0066/0067 predecessor chain is frozen.
...
BEGIN
  IF current_user = 'learncoding_owner'
    ...
    IF redaction_disposition IN (
       ...
       AND OLD.idempotency_payload_sha256
             IS NOT DISTINCT FROM NEW.idempotency_payload_sha256
  ...
  IF OLD.idempotency_authority_sha256 IS DISTINCT FROM
    ...
    RAISE EXCEPTION 'email_outbox.idempotency_authority_sha256 is immutable'
      USING ERRCODE = '23514';
  END IF;
  IF OLD.idempotency_payload_sha256 IS DISTINCT FROM
       NEW.idempotency_payload_sha256 THEN
    RAISE EXCEPTION 'email_outbox.idempotency_payload_sha256 is immutable'
      USING ERRCODE = '23514';
  END IF;
