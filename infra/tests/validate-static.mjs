...
const mailRetentionPrivacyIntegration = read(
  "integration/mail-retention-privacy.integration.test.ts",
);
...
expect(
  /unresolvedEmailDeliveryAuthority:[\s\S]*?redact_pii_retain_correlation_authority/.test(
    ...
    /email_outbox_idempotency_coverage_authority/.test(retentionRuntime) &&
    /savepoint retention_email_dedup_coverage/.test(retentionRuntime) &&
    /categories\.terminalEmailDeliveryRecords\?\.outcome === "failed"/.test(
      retentionRuntime,
    ) &&
    /categories\.unresolvedEmailDeliveryAuthorityBlocked\?\.eligible/.test(
      retentionRuntime,
    ) &&
    /categories\.unclassifiedEmailDeliveryAuthorityBlocked\?\.eligible/.test(
      retentionRuntime,
    ) &&
    /expired complete Gmail claim retry-required/.test(mailRetentionPrivacyIntegration) &&
    /remediationKey/.test(mailRetentionPrivacyIntegration) &&
    ...
    /strict-true durable hashed no-replay coverage/.test(lifecycleRunbook) &&
    /replaying the completed-with-errors key is not remediation/.test(
      lifecycleRunbook,
    ) &&
    ...
    /deleting the outbox row can never remove the last durable no-replay barrier/.test(
      privacyInventory,
    ) &&
    /policy v5 is not standalone privacy or terminal-deletion closure/.test(
      privacyInventory,
    ) &&
    !/Terminal sent\/suppressed\/failed\/quarantined email delivery records/.test(
      lifecycleRunbook,
    ),
  "v5 policy, runtime, runbook, and privacy inventory must agree on exact deletion partitions, blocked-PII retry health, hashed no-replay coverage, and the 0067 closure dependency",
);
expect(
