...
function walk(directory, repositoryRoot, output) {
  for (const entry of readdirSync(directory, { withFileTypes: true })) {
    ...
    if (
      PRODUCTION_EXTENSIONS.includes(extension)
      && !TEST_FILE.test(stem)
    ) {
...
function sourceWithoutComments(source, extension) {
  ...
  const withoutBlockComments = extension === ".sql"
    ...
    if (ts.isCallExpression(node)) {
      ...
      if (
        ...
        if (
          ...
          && insertCall.arguments.some((argument) => (
...
function dispatchAuthoritySource(source) {
  ...
  return sourceFile.statements
    ...
    .filter((statement) => (
      statement.name
      && DISPATCH_POLICY_FUNCTIONS.has(statement.name.text)
    ))
...
export function extractDispatchEnabledTemplates(source) {
  const templates = [];
  const policySource = sourceWithoutComments(dispatchAuthoritySource(source), ".ts");
  for (const match of policySource.matchAll(/\btemplate\s*=\s*'([a-z][a-z0-9-]*)'/giu)) {
    templates.push(match[1]);
  }
  for (const match of policySource.matchAll(/\btemplate\s+in\s*\(([^)]*)\)/giu)) {
...
export function auditMailWriterInventory(repositoryRoot, options = {}) {
  ...
  const sources = new Map(productionFiles.map((path) => {
  ...
  const centralWriterPath = options.centralWriterPath === undefined
    ? "src/lib/notifications/outbox.ts"
  ...
  if (dispatchEnabledTemplates === undefined) {
    ...
    dispatchEnabledTemplates = policy
      ? extractDispatchEnabledTemplates(policy.source)
      : [];
  }
  ...
  for (const writer of directWriters) {
    if (writer.kind === "sql") {
      for (const field of missingFields(writer.columns, SQL_REQUIRED_COLUMNS)) {
        errors.push(`${writer.path}: direct SQL email_outbox writer is missing ${field}`);
      }
    ...
    for (const field of missingFields(writer.properties, ORM_REQUIRED_PROPERTIES)) {
      errors.push(`${writer.path}: direct ORM email_outbox writer is missing ${field}`);
    }
  }
  ...
  if (centralWriterPath !== null) {
    ...
    if (!central) {
      errors.push(`central email_outbox writer is missing: ${centralWriterPath}`);
    } else {
      ...
      for (const fragment of requiredFragments) {
        if (!central.source.includes(fragment)) {
          errors.push(`${centralWriterPath}: central writer is missing ${fragment}`);
        }
      }
    }
  }
  ...
  for (const producer of reviewedTemplateProducers) {
    ...
    const hasTemplate = evidence.javascript.templateProperties.has(
      producer.template,
    ) || evidence.writers.some((writer) =>
      directWriterTemplates(writer).includes(producer.template));
    const hasCall = producer.call === "direct-sql"
      ? evidence.writers.some((writer) => (
          writer.kind === "sql"
          && directWriterTemplates(writer).includes(producer.template)
        ))
  ...
  for (const template of reviewedDispatchTemplates) {
    if (!actualDispatchSet.has(template)) {
      errors.push(`reviewed dispatch template is no longer enabled: ${template}`);
    }
  }
