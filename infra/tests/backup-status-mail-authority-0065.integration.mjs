...
async function main() {
  ...
  const temporaryRoot = managedServer
    ...
    : undefined;
  const socketDirectory = temporaryRoot
    ? path.join(temporaryRoot, "socket")
    : undefined;
  if (socketDirectory !== undefined) mkdirSync(socketDirectory);
  const socketOption =
    socketDirectory === undefined || process.platform === "win32"
      ? ""
      : ` -k "${socketDirectory}"`;
    ...
    if (managedServer) {
      ...
      run(
        ...
        [
          ...
          serverLog,
          "--options",
          `-h 127.0.0.1 -p ${port} -F -c listen_addresses=127.0.0.1${socketOption}`,
          "--no-wait",
          "start",
        ],
