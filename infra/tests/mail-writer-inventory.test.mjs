...
const inventoryModule = await import("./lib/mail-writer-inventory.mjs")
  .catch(() => null);
...
test("fails closed for unreviewed writers and producerless dispatch templates", () => {
  ...
  try {
    ...
    assert.match(errors, /unreviewed direct email_outbox writer: src\/unreviewed\.ts/u);
    assert.match(errors, /dispatch-enabled template has no reviewed production producer: reset-password/u);
  } finally {
    repo.cleanup();
  }
});
...
test("does not inventory comments, test fixtures, or migration fixtures as writers", () => {
  ...
  const repo = fixture({
    ...
    `,
    "src/comment.py": "# INSERT INTO email_outbox (operation_id) VALUES ('x')\n",
...
test("default inventory keeps the host backup writer and every dispatch template attributable", () => {
  const inventory = requireInventoryModule();
  assert.ok(
  ...
  for (const template of inventory.REVIEWED_DISPATCH_ENABLED_TEMPLATES) {
    assert.ok(templates.has(template), `${template} must have a reviewed producer`);
  }
});
