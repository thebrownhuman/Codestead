...
- `database_backup_reporter_url`: the backup-status enqueue URL for fixed user `learncoding_backup_reporter`; it can execute only the reviewed enqueue routine and has no table or sequence rights.
...
Initial creation only: create the seven database files with `sudo -H bash /opt/learncoding/infra/ops/create-database-secrets.sh`. The ceremony locks the directory, refuses any pre-existing final, stages and validates all seven files before publication, and removes every final created by a failed run. It generates six distinct hexadecimal passwords, writes every database secret without a trailing newline, whitespace, or control byte, never prints a value, unsets its temporary variables, and sets the directory to `root:codestead-secrets` mode `0750` and the seven files to `root:codestead-secrets` mode `0440`. Use a separate reviewed rotation procedure for live credentials.
