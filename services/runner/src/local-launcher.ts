...
const RUNTIME_RECORD_LANGUAGES = ["c", "cpp", "java", "python", "javascript"] as const;
const OCI_DIGEST = /^sha256:[a-f0-9]{64}$/;
const SHA256_HEX = /^[a-f0-9]{64}$/;

type RuntimeRecordLanguage = typeof RUNTIME_RECORD_LANGUAGES[number];

interface RuntimeImageRecord {
  readonly language: RuntimeRecordLanguage;
...
export function validateRuntimeImageRecordText(input: {
  readonly envText: string;
  readonly jsonText: string;
}): ParsedEnvironment {
  let candidate: unknown;
  try {
    candidate = JSON.parse(input.jsonText);
  } catch {
    throw new Error("Runtime image record JSON commit marker is missing or invalid.");
  }
  if (
    !isObject(candidate)
    || !hasExactKeys(candidate, ["schemaVersion", "recordId", "release", "local", "records"])
    || candidate.schemaVersion !== 1
    || typeof candidate.recordId !== "string"
    || !SHA256_HEX.test(candidate.recordId)
    || typeof candidate.release !== "string"
    || candidate.release.length === 0
    || typeof candidate.local !== "boolean"
    || !Array.isArray(candidate.records)
    || candidate.records.length !== RUNTIME_RECORD_LANGUAGES.length
  ) {
    throw new Error("Runtime image record has an unsupported or non-canonical schema or record id.");
  }

  const records: RuntimeImageRecord[] = candidate.records.map((value, index) => {
    ...
    const expectedLanguage = RUNTIME_RECORD_LANGUAGES[index];
    if (!expectedLanguage) {
      throw new Error("Runtime image record language set is incomplete.");
    }
    if (
      value.language !== expectedLanguage
      || typeof value.reference !== "string"
      || typeof value.manifestDigest !== "string"
      || !OCI_DIGEST.test(value.manifestDigest)
      || !value.reference.endsWith(`@${value.manifestDigest}`)
      || typeof value.configDigest !== "string"
      || !OCI_DIGEST.test(value.configDigest)
      || value.configDigest === value.manifestDigest
      || typeof value.rootDigest !== "string"
      || !OCI_DIGEST.test(value.rootDigest)
      || (candidate.local && value.rootDigest !== value.manifestDigest)
    ) {
      throw new Error("Runtime image record has a non-canonical language, reference, or config digest.");
    }
    return {
  ...
  const expectedRecordId = createHash("sha256")
    .update(JSON.stringify(payload))
    .digest("hex");
  if (candidate.recordId !== expectedRecordId) {
...
export function buildLocalRunnerEnvironment(input: {
  ...
  for (const key of RUNNER_IMAGE_KEYS) {
    const value = input.imageEnv[key];
    if (!value) throw new Error(`${key} is missing from the canonical dist/runtime-images publication.`);
    const inheritedValue = input.inheritedEnv[key];
    if (inheritedValue !== undefined && inheritedValue !== "" && inheritedValue !== value) {
      throw new Error(`${key} cannot override the canonical dist/runtime-images publication.`);
    }
    childEnv[key] = value;
  }
  return childEnv;
...
export async function startLocalRunner(): Promise<number> {
  ...
  try {
    const env = buildLocalRunnerEnvironment({
      ...
      imageEnv: validateRuntimeImageRecordText({
        envText: readFileSync(path.join(runnerRoot, "dist", "runtime-images.env"), "utf8"),
        jsonText: readFileSync(path.join(runnerRoot, "dist", "runtime-images.json"), "utf8"),
      }),
      stateRoot,
      tempRoot,
      lockFd: 3,
