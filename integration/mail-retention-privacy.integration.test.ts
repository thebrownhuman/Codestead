...
type AuthorityShape = "released" | "active" | "expired" | "partial";
...
describe("mail retention privacy policy on PostgreSQL 17", () => {
    ...
    await owner.query(
      ...
      UPDATE public.email_outbox
         SET status = 'quarantined',
             claim_version = 2,
             claim_token = CASE
               WHEN $3::text IN ('active', 'expired', 'partial') THEN $2::uuid
               ELSE NULL
             END,
             claim_owner = CASE
               WHEN $3::text IN ('active', 'expired') THEN 'mail-retention-privacy-pg17'
               ELSE NULL
             END,
  ...
  it("keeps an expired complete Gmail claim retry-required while recipient PII remains", async () => {
    const expiredGmailClaim = fixture(328, "gmail", "expired");
    await insertAuthority(expiredGmailClaim);

    const idempotencyKey = "retention:privacy-pg17:expired-gmail-claim";
    ...
    expect(report).toMatchObject({
      ...
      requiresRetry: true,
      categories: {
        terminalEmailDeliveryRecords: { eligible: 0, deleted: 0 },
        unresolvedEmailDeliveryAuthority: { eligible: 0, transitioned: 0 },
        unresolvedEmailDeliveryAuthorityBlocked: { eligible: 1, retained: 1 },
        unresolvedEmailDeliveryAuthorityMalformed: { eligible: 0, retained: 0 },
        unclassifiedEmailDeliveryAuthorityBlocked: { eligible: 0, retained: 0 },
    ...
    const retained = await owner.query<{
      ...
      `
      SELECT status::text, to_email, claim_token::text, claim_owner,
             lease_expires_at, provider_message_id, sent_at
        FROM public.email_outbox
       WHERE id = $1::uuid
      `,
      [expiredGmailClaim.id],
    );
    expect(retained.rows[0]).toMatchObject({
      status: "quarantined",
      to_email: expiredGmailClaim.recipient,
      claim_token: expiredGmailClaim.claimToken,
      claim_owner: "mail-retention-privacy-pg17",
      lease_expires_at: expect.any(Date),
      provider_message_id: null,
      sent_at: null,
    });
    expect(retained.rows[0]?.lease_expires_at?.getTime()).toBeLessThan(
      NOW.getTime(),
    );
    ...
    const remediationKey = "retention:privacy-pg17:expired-gmail-claim:released";
    runKeys.add(remediationKey);
    const remediated = await runRetention({
    ...
    expect(remediated).toMatchObject({
      ...
      categories: {
        ...
        unresolvedEmailDeliveryAuthorityBlocked: { eligible: 0, retained: 0 },
        unresolvedEmailDeliveryAuthorityMalformed: { eligible: 0, retained: 0 },
      },
    });

    const redacted = await owner.query<{
      status: string;
      to_email: string;
      claim_token: string | null;
      claim_owner: string | null;
      ...
      `
      SELECT status::text, to_email, claim_token::text, claim_owner, lease_expires_at
        FROM public.email_outbox
       WHERE id = $1::uuid
      `,
      [expiredGmailClaim.id],
    ...
    expect(redacted.rows[0]).toMatchObject({
      status: "quarantined",
      to_email: `redacted+${expiredGmailClaim.id}@invalid.local`,
      claim_token: null,
      claim_owner: null,
      lease_expires_at: null,
    });
  ...
  it("gives each current or allowed historical old quarantine one privacy disposition", async () => {
    ...
    expect(byId.get(consoleReleased.id)).toMatchObject({
      status: "quarantined",
      to_email: consoleReleased.recipient,
      claim_token: null,
      claim_owner: null,
      lease_expires_at: null,
      provider_message_id: null,
    ...
    expect(byId.get(gmailFullReceipt.id)).toMatchObject({
      status: "quarantined",
      to_email: gmailFullReceipt.recipient,
      claim_token: null,
      claim_owner: null,
      lease_expires_at: null,
      provider_message_id: gmailFullReceipt.providerMessageId,
