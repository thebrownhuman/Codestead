...
import type { GmailReconciliationDispatchBinding } from "./gmail-reconciliation";
...
export async function findGmailMessageByMessageId(
  ...
  const validExpectedBinding =
    expectedBinding.kind === "legacy-unbound"
      ? expectedBinding.bindingVersion === null &&
        expectedBinding.bindingSha256 === null
      : expectedBinding.kind === "exact-bound" &&
        expectedBinding.bindingVersion === "gmail-raw-v1" &&
        /^[0-9a-f]{64}$/.test(expectedBinding.bindingSha256);
  if (!validExpectedBinding) {
  ...
  if (expectedBinding.kind === "exact-bound") {
    ...
    const rawMessage = await withGmailRequestDeadline(
      "reconciliation",
      requestTimeoutMs,
      async (signal) => {
        ...
        if (!response.ok) {
          throw new Error(
            `Gmail reconciliation raw verification failed (${response.status}).`,
          );
        }
        return response.json() as Promise<unknown>;
      },
    ...
    const rawLabelIds = rawRecord.labelIds;
    const encodedRfc822 = rawRecord.raw;
    if (
      typeof rawRecord.id !== "string" ||
      rawRecord.id.trim() !== providerMessageId ||
      !Array.isArray(rawLabelIds) ||
      rawLabelIds.some((label) => typeof label !== "string") ||
      !rawLabelIds.includes("SENT") ||
      typeof encodedRfc822 !== "string" ||
      encodedRfc822.length === 0 ||
      encodedRfc822.length > 50_000_000 ||
      !/^[A-Za-z0-9_-]+$/.test(encodedRfc822)
    ) {
      return { kind: "ambiguous" as const };
    }
    const canonicalEncodedRfc822 = encodedRfc822;
    let rfc822Bytes: Buffer;
    let rfc822: string;
    try {
      rfc822Bytes = Buffer.from(canonicalEncodedRfc822, "base64url");
      if (
        rfc822Bytes.length === 0 ||
        rfc822Bytes.toString("base64url") !== canonicalEncodedRfc822
      ) {
        return { kind: "ambiguous" as const };
      }
      rfc822 = new TextDecoder("utf-8", { fatal: true }).decode(rfc822Bytes);
      if (!Buffer.from(rfc822, "utf8").equals(rfc822Bytes)) {
        return { kind: "ambiguous" as const };
      }
    } catch {
      return { kind: "ambiguous" as const };
    }
    const observedBindingSha256 = createHash("sha256")
      .update(rfc822, "utf8")
      .digest("hex") as typeof expectedBinding.bindingSha256;
    if (observedBindingSha256 !== expectedBinding.bindingSha256) {
      return { kind: "ambiguous" as const };
    }
    return {
      ...
      bindingEvidence: Object.freeze({
        ...
        bindingSha256: observedBindingSha256,
      }),
    };
  }
