...
describe("email outbox", () => {
  ...
  it("normalizes the recipient and derives a deterministic non-secret idempotency key", async () => {
    const input = {
      ...
      variables: { name: "Learner", url: "https://example.test/activate?token=one-time-secret" },
      userId: "learner-1",
      idempotencySeed: "invitation-1",
    };
    await enqueueEmail(input);

    const expected = createHash("sha256")
      .update("invitation:learner@example.com:invitation-1")
      .digest("hex");
    ...
    expect(mocks.onConflictDoNothing).toHaveBeenCalledWith({ target: "idempotency_key" });
  });
