...
const PAYLOAD_SHA256 = "b".repeat(64) as ProviderPayloadSha256;
...
describe("dispatch evidence digest", () => {
  ...
  it("binds both the final adapter bytes and the exact random header token", () => {
    ...
    expect(dispatchEvidenceSha256(evidenceInput({
      adapterPayloadSha256: "c".repeat(64) as ProviderPayloadSha256,
    }))).not.toBe(canonical);
    expect(dispatchEvidenceSha256(evidenceInput({
  ...
  it("rejects extra and symbol keys", () => {
    expect(() => dispatchEvidenceSha256({
      ...evidenceInput(),
      extra: "not-framed",
    } as DispatchEvidenceInput)).toThrow(
    ...
    expect(() => dispatchEvidenceSha256(Object.assign(
      evidenceInput(),
      { [Symbol("hidden")]: "not-framed" },
    ))).toThrow("Dispatch evidence input is invalid.");
  });
});
