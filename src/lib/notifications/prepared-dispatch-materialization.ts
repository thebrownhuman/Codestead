...
export type PreparedDispatchStoreInspection = Readonly<{
  ...
  providerEvidenceSha256: DispatchEvidenceSha256 | null;
}>;
...
>();
const AUTHORIZATION_ELIGIBLE_ENVELOPES = new WeakSet<PreparedDispatchEnvelope>();
...
function canonicalRuntimePlan(value: unknown): PreparedDispatchRuntimePlan {
  try {
    ...
    if (!timeouts || typeof timeouts !== "object" || !Object.isFrozen(timeouts)) {
      throw new Error("invalid");
    }
...
export function createMaterializedDispatch(input: Readonly<{
  ...
  const evidenceToken = prepared.adapter === "gmail"
    ? preparedGmailEvidenceToken(prepared.rfc822)
    : null;
  const providerEvidenceSha256 = prepared.adapter === "gmail"
    ? dispatchEvidenceSha256({
        operationId: source.operationId,
  ...
  PREPARED_DISPATCH_STORE_STATES.set(storeView, Object.freeze({
    ...
    ...(rendered.authorityEvidence
      ...
      : {}),
    providerCorrelationVersion: PROVIDER_CORRELATION_VERSION,
    providerEvidenceVersion: prepared.adapter === "gmail"
      ? PROVIDER_EVIDENCE_VERSION
      : null,
...
export function createStoreBoundPreparedDispatchChannel(
  ...
  const discardReceipt = (
    ...
    return committed.permit === permit
      && dispatch.runtimePlan === canonicalPlan
      && dispatch.storeView === committed.view;
  };
  const discardGuard = (
