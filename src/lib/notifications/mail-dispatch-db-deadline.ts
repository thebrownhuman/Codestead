...
export type MailDispatchDbDeadlinePhase =
  "pool-acquire" | "pre-provider" | "post-init-arm" | "post-provider";
...
export class MailDispatchDbDeadline {
  ...
  constructor(
    ...
    if (
      ...
      throw new Error(
        "Mail dispatch database deadline is outside the safe range.",
      );
    }
  }
...
export function createCappedMailDispatchDbDeadline(
  input: Readonly<{
    parent: MailDispatchDbDeadline;
    budgetMs: number;
    phase?: MailDispatchDbDeadlinePhase;
...
> {
  ...
  #released = false;
  #clientEnded = false;
  readonly #clientErrorListener: ((error: unknown) => void) | undefined;
  readonly #clientEndListener: (() => void) | undefined;

  constructor(client: Client) {
    this.client = client;
    const on = client.on;
    const once = client.once;
    const removeListener = client.removeListener;
    this.#clientErrorListener =
      typeof on === "function" && typeof removeListener === "function"
        ? () => undefined
        : undefined;
    if (this.#clientErrorListener) {
      // pg removes its pool listener while a client is checked out. Queries still
      // reject normally; this listener only prevents a second EventEmitter error
      // from terminating the worker before uncertainty handling can run.
      on!.call(client, "error", this.#clientErrorListener);
    }
    this.#clientEndListener =
      typeof once === "function" && typeof removeListener === "function"
        ? () => {
            this.#clientEnded = true;
          }
        : undefined;
    if (this.#clientEndListener) {
      once!.call(client, "end", this.#clientEndListener);
    }
  }
  ...
  #close(destroy: boolean, beforeDestroy?: () => void): void {
    ...
    try {
      ...
      const listener = this.#clientErrorListener;
      const removeListener = this.client.removeListener;
      if (listener && typeof removeListener === "function") {
        // node-postgres installs its own listener synchronously inside release().
        removeListener.call(this.client, "error", listener);
      }
      if (this.#clientEndListener && typeof removeListener === "function") {
        removeListener.call(this.client, "end", this.#clientEndListener);
      }
    }
