...
import { consumeLiveProviderTx2Authority } from "./postgres-outbox-store";
...
function deliveryError(error: unknown, code: MailDeliveryFailureCode) {
...
export function captureMailTransportConfiguration(
  ...
  MAIL_TRANSPORT_CONFIGURATIONS.set(
    handle,
    Object.freeze({
      adapter,
...
export function capturePreparedMailTransportPlan(
  ...
  if (
    ...
    timing.providerAbortSettlementMs !== GMAIL_ABORT_SETTLEMENT_RESERVE_MS
  ) {
  ...
  if (configurationState.adapter === "console") {
    PREPARED_MAIL_TRANSPORT_PLANS.set(
      handle,
      Object.freeze({
...
async function gmailAccessToken(
  ...
  const clientSecret =
    configuration?.clientSecret ?? process.env.GMAIL_CLIENT_SECRET;
  const refreshToken =
    configuration?.refreshToken ?? process.env.GMAIL_REFRESH_TOKEN;
  if (!clientId || !clientSecret || !refreshToken)
    throw new Error("Gmail OAuth is not configured.");
  return withGmailRequestDeadline(
    "OAuth",
    requestTimeoutMs,
    async (signal) => {
...
export async function findGmailMessageByMessageId(messageId: string) {
  ...
  const listBody = await withGmailRequestDeadline(
    ...
    async (signal) => {
      ...
      if (!response.ok) {
        throw new Error(
          `Gmail reconciliation search failed (${response.status}).`,
        );
      }
  ...
  if (providerIds.length !== 1 || providerIds.length !== messages.length) {
    return { kind: "ambiguous" as const };
  }
  ...
  const metadata = await withGmailRequestDeadline(
    ...
    async (signal) => {
      ...
      if (!response.ok) {
        throw new Error(
          `Gmail reconciliation verification failed (${response.status}).`,
        );
      }
      return response.json() as Promise<unknown>;
    },
  ...
  const messageIdHeaders = rawHeaders
    ...
    .filter(
      ({ name }) =>
        typeof name === "string" && name.toLowerCase() === "message-id",
    )
...
export async function authorizePreparedEmail(
  ...
  if (!preparedEmailBindingMatches(preparedSnapshot, authoritySnapshot)) {
  ...
  const oauthRequestTimeoutMs = plan.timing.oauthDeadlineMs - abortSettlementMs;
...
function assertPreparedGmailAuthorization(
  ...
  if (
    ...
    authorization.abortSettlementMs !== GMAIL_ABORT_SETTLEMENT_RESERVE_MS ||
...
function snapshotPreparedEmail(prepared: PreparedEmail): PreparedEmail {
  ...
  if (adapter === "gmail") {
    ...
    return Object.freeze({
      ...
      requestBodySha256: gmail.requestBodySha256,
      requestBodyLength: gmail.requestBodyLength,
    });
  }
  if (adapter === "console") {
    ...
    return Object.freeze({
      ...
      requestBodySha256: consoleEmail.requestBodySha256,
      requestBodyLength: consoleEmail.requestBodyLength,
...
const CONSOLE_WRITE_DEADLINE_MS = 20_000;

function settleConsoleWrite(
  eventBytes: string,
  externalSignal?: AbortSignal,
): Promise<void> {
  if (externalSignal?.aborted) {
    return Promise.reject(new MailDeliveryError("MAIL_PRE_SEND_REJECTED"));
  }
  return new Promise((resolve, reject) => {
    ...
    const ambiguous = () => new MailDeliveryError("PROVIDER_OUTCOME_AMBIGUOUS");
    const onAbort = () => finish(ambiguous());
    const timer = setTimeout(
      () => finish(ambiguous()),
      CONSOLE_WRITE_DEADLINE_MS,
    );
    ...
    try {
      process.stdout.write(eventBytes, "utf8", (error) => {
        finish(error ? ambiguous() : undefined);
      });
    } catch {
      finish(ambiguous());
    }
...
export async function sendPreparedEmail(
  ...
  liveTx2Context: unknown,
  authorization: PreparedEmailAuthorization,
  options: PreparedEmailSendOptions = {},
) {
  if (!consumeLiveProviderTx2Authority(liveTx2Authority, liveTx2Context)) {
  ...
  if (authorizationSnapshot.adapter === "console") {
    ...
    await settleConsoleWrite(consolePrepared.eventBytes, externalSignal);
    ...
    return await withGmailRequestDeadline(
      ...
      async (signal) => {
        ...
        if (!response.ok) {
          ...
          if (
            [400, 401, 403, 404, 405, 410, 413, 415, 422].includes(
              response.status,
            )
          ) {
