...
export function isFatalProviderTransportError(
  ...
  ) && FATAL_PROVIDER_TRANSPORT_STATES.has(
    error as FatalProviderTransportError,
  );
}
...
export function fatalProviderTransportCode(
  ...
  return FATAL_PROVIDER_TRANSPORT_STATES.get(error)
    ?? "PROVIDER_TRANSPORT_FATAL";
}
...
export class MailDeliveryError extends Error {
  constructor(code: MailDeliveryFailureCode) {
    ...
    const canonicalCode = typeof code === "string"
      && Object.hasOwn(MAIL_DELIVERY_FAILURES, code)
      ? code
      : "PROVIDER_OUTCOME_UNKNOWN";
    MAIL_DELIVERY_ERROR_STATES.set(
      this,
      MAIL_DELIVERY_FAILURES[canonicalCode],
    );
  }
}
