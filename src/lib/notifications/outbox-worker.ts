...
async function beginFatalShutdown<P, M, A>(
  ...
  try {
    ...
    // Hard exit remains mandatory even when transport teardown reports failure.
  }
  try {
  ...
  for (let index = 0; index < deps.policy.batchSize; index += 1) {
    ...
    try {
      ...
      if (error instanceof FatalProviderTransportError) {
        return await beginFatalShutdown(deps, error);
      }
      ...
      const guarded = await deps.store.dispatchAfterProviderBoundary(
        boundary.permit,
        {
          ...
          invoke: async (signal) => {
            ...
            if (providerResult.kind === "fatal") {
              return await beginFatalShutdown(
                deps,
                new FatalProviderTransportError(providerResult.code),
              );
            }
