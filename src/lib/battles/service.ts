...
async function resolveCreateUserCandidates(
...
): Promise<CreateUserCandidates | null> {
  ...
  const invitees: CreateUserCandidate[] = [];
  for (const publicId of invitedPublicIds) {
    const candidate = byPublicId.get(publicId);
    if (!candidate) return null;
    invitees.push({ id: candidate.id, publicId });
  }
...
async function hasCreateReplayHint(
  client: PoolClient,
  actorUserId: string,
  requestId: string,
) {
  const result = await client.query(
    `select 1 from coding_battle
      where creator_user_id=$1 and create_request_id=$2`,
...
async function lockAndReadCreateReplay(
  ...
  await client.query(
    "select pg_advisory_xact_lock(hashtext($1))",
    [`battle-create:${actorUserId}:${requestId}`],
  );
  ...
  if (replay.create_input_hash !== fingerprint) {
...
export async function createBattle(input: {
  ...
  try {
    ...
    const initialReplayHint = await hasCreateReplayHint(
      client,
      input.actorUserId,
      input.requestId,
    );
    if (initialReplayHint) {
      await lockUserAuthorities(client, [input.actorUserId]);
      const actor = await activeActor(client, input.actorUserId, true);
      if (competition && actor.role !== "admin") {
        throw new BattleError("NOT_FOUND");
      }
      const replay = await lockAndReadCreateReplay(
        client,
        actor.id,
        input.requestId,
        fingerprint,
      );
      if (!replay) throw new BattleError("INTEGRITY_FAILURE");
    ...
    if (await hasCreateReplayHint(client, input.actorUserId, input.requestId)) {
      const actor = await activeActor(client, input.actorUserId, true);
      if (competition && actor.role !== "admin") {
        throw new BattleError("NOT_FOUND");
      }
