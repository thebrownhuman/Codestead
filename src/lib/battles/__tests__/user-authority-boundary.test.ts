...
describe("battle user-authority ordering", () => {
  ...
  it("uses actor-only exact replay and otherwise fails closed when candidates disappear", () => {
    const create = functionBody(
      "export async function createBattle",
      "async function loadAccessibleBattle",
    );
    const missingBranch = create.slice(
      create.indexOf("if (!candidates)"),
      create.indexOf("candidates.actorUserId"),
    );
    const actorAuthority = missingBranch.indexOf(
      "await lockUserAuthorities(client, [input.actorUserId])",
    );
    const actorRow = missingBranch.indexOf(
      "await activeActor(client, input.actorUserId, true)",
    );
    const exactReplay = missingBranch.indexOf(
      "await lockAndReadCreateReplay",
    );
    const failClosed = missingBranch.indexOf(
      'if (!replay) throw new BattleError("NOT_FOUND")',
    );

    expect(actorAuthority).toBeGreaterThanOrEqual(0);
    expect(actorRow).toBeGreaterThan(actorAuthority);
    expect(exactReplay).toBeGreaterThan(actorRow);
    expect(failClosed).toBeGreaterThan(exactReplay);
    expect(missingBranch).not.toContain("lockCreateUserRows");
    expect(missingBranch).not.toContain("activeInvitees");
  });
