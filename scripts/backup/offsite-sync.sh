...
archive_bytes="$(stat -c '%s' -- "$archive_path")" \
  || die "marked archive size could not be read"
...
verify_ciphertext_checksum_service_budget "$archive_path" "$archive_bytes" \
...
verify_ciphertext_checksum_service_budget "$readback_dir/$archive" "$archive_bytes" \
  || die "offsite read-back checksum verification failed"
run_backup_work_bulk "$archive_bytes" \
...
if ((${#point_entries[@]} == 0)); then
  ...
  run_rclone_finalization_control copyto "$point_candidate" "$pending_point" \
    || die "offsite point-attestation upload was not confirmed"
  run_rclone_finalization_control moveto "$pending_point" "$remote_point" \
    || die "offsite point-attestation publication was not confirmed"
else
  point_candidate="$stage/point.env"
  run_rclone_finalization_control copyto "$remote_point" "$point_candidate" \
...
run_rclone_finalization_control copyto "$remote_point" "$point_readback" \
...
run_rclone_finalization_control copyto "$point_readback" "$pending_pointer" \
  || die "offsite pointer upload was not confirmed"
run_rclone_finalization_control moveto "$pending_pointer" "$remote_pointer" \
  || die "offsite pointer publication was not confirmed"
pointer_readback="$stage/pointer-readback.env"
run_rclone_finalization_control copyto "$remote_pointer" "$pointer_readback" \
...
require_rclone_service_finalization_budget \
  || die "offsite deadline expired before the final success sequence"
emit_alert info offsite_sync_complete \
  "encrypted recovery point upload and read-back verification completed"
require_rclone_service_finalization_budget \
  || die "offsite deadline expired while reporting verified publication"

local_offsite_marker="$backup_root/state/offsite-last-success.env"
write_success_marker "$local_offsite_marker" \
  "$SUCCESS_ARCHIVE" "$SUCCESS_COMPLETED_UTC" "$SUCCESS_SHA256" \
  || die "local offsite acknowledgement could not be committed"
if ! cmp -s -- "$local_offsite_marker" "$pointer_readback"; then
  rm -f -- "$local_offsite_marker"
  die "local offsite acknowledgement differs from remote verified bytes"
fi
if ! require_rclone_service_finalization_budget; then
  rm -f -- "$local_offsite_marker"
  die "offsite deadline expired while committing local success"
fi
