...
describe("full-schema restore active redaction authority", () => {
  it("uses the v2 redactor for the suppression control when it is installed", async () => {
...
describe("full-schema restore catalog and row snapshot", () => {
  it("hashes recursively canonicalized data without depending on object key order", () => {
    expect(
      stableSha256([{ z: 1, nested: { beta: true, alpha: null }, a: "x" }]),
    ).toBe(
      stableSha256([{ a: "x", nested: { alpha: null, beta: true }, z: 1 }]),
    );
    expect(stableSha256([{ a: 1 }, { a: 2 }])).not.toBe(
      stableSha256([{ a: 2 }, { a: 1 }]),
    );
  });
  ...
  it("collects a dynamic journal tail, complete object contract, and future-column mail rows", async () => {
    ...
    expect(queries[2]).toContain("namespace.nspname in ('public', 'drizzle')");
    ...
    expect(queries[3]).not.toMatch(
      /select\s+[a-z_, ]+\s+from public\.email_outbox/iu,
    );
  });
  ...
  it("fails closed instead of hashing a missing journal or empty fixture set", async () => {
    ...
    await expect(collectFullSchemaRestoreSnapshot(client)).rejects.toThrow(
      "full-schema restore database snapshot failed",
    );
  });
  ...
  const baseObject = {
    ...
    attributes: {
      ...
      definition:
        "CREATE FUNCTION authority(uuid) RETURNS boolean AS 'SELECT true'",
      policies: [],
    },
  } as const;
  ...
  it.each([
    [
      ...
      {
        definition:
          "CREATE FUNCTION authority(uuid) RETURNS boolean AS 'SELECT false'",
      },
    ],
    [
      "index definition",
      {
        definition:
          "CREATE UNIQUE INDEX authority_idx ON authority (id, operation_id)",
      },
    ],
    [
      "view definition",
    ...
    [
      "range semantics",
      {
        range: {
          ...
          canonical: null,
          subdiff:
            "pg_catalog.tstzrange_subdiff(timestamp with time zone,timestamp with time zone)",
        },
      },
    ],
    [
  ...
  it("queries and fingerprints exact sequence last_value and is_called state", async () => {
    ...
    expect(uncalled.result.objectContractSha256).not.toBe(
      called.result.objectContractSha256,
    );
  });
});
...
describe("full-schema restore SQL-only smoke", () => {
  it("rolls back the worker claim probe and commits ops redaction without any provider call", async () => {
    ...
    expect(
      workerTrace.some((sql) =>
        sql.includes("pg_catalog.pg_advisory_xact_lock"),
      ),
    ).toBe(true);
    ...
    expect(
      [...workerTrace, ...opsTrace, ...verifierTrace].join("\n"),
  ...
  it.each([
    ...
    "rejects a mutated restored system envelope field %s",
    async (field, value) => {
      const worker: FullSchemaRestoreQueryClient = {
        query: vi.fn(async (sql: string) => {
          if (sql.includes("returning id")) {
  ...
  it("uses SHA-256 rather than a weak catalog checksum", () => {
    expect(stableSha256({ value: "catalog" })).toBe(
      createHash("sha256").update('{"value":"catalog"}', "utf8").digest("hex"),
    );
  });
});
