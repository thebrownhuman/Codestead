...
const roleUrls = {
  ...
  backupReporter:
    "postgresql://learncoding_backup_reporter:reporter-password-0005@127.0.0.1:49152/learncoding_integration",
} as const;
...
const roleProperties = [
  ...
  ["learncoding_backup_reporter", "databaseBackupReporterUrl", "backupReporter"],
] as const;
...
describe("disposable role-boundary adapter", () => {
  it("uses exact canonical verifier URLs and rewrites only pool host and port", async () => {
    ...
    expect(capturedVerifierInput).toMatchObject({
      ...
      databaseBackupReporterUrl:
        "postgresql://learncoding_backup_reporter:reporter-password-0005@postgres:5432/learncoding_integration",
    });
