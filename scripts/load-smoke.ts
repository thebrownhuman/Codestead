...
async function sample(url: URL, timeoutMs: number, cookie?: string): Promise<RequestSample> {
...
async function main() {
  ...
  for (const scenario of scenarios) {
    ...
    const warmup = await sample(target, warmupTimeoutMs, cookie); // Excluded from latency percentiles.
    if (!warmup.ok) {
      failed = true;
    ...
    const jobs = Array.from({ length: requestsPerScenario }, () => () => sample(target, timeoutMs, cookie));
